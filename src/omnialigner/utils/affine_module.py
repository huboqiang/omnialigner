...
def create_initial_grid(tfrs_module: AffineModule, **kwargs) -> Tensor_grid_2d:
    grid = tfrs_module.forward(None)
    return grid
...
class AffineEngineRawData(nn.Module):
    def __init__(self, 
        ...
        self.load_tfrs(init_tfrs=init_tfrs)
        self.grid_up = self.calculate_grid(size=params_hw)
    
    def load_tfrs(self, init_tfrs: Tensor_tfrs):
        self.tfrs_module = AffineModule(init_tfrs, tensor_size=self.params_hw)
        height, width = self.params_hw
        self.grid_affine = create_initial_grid(self.tfrs_module)
        
    def export_tfrs(self) -> Tensor_tfrs:
