    ...
    def match_batched(
        ...
        *args,
        ...
        with torch.no_grad():
            ...
            batch_resized = {
                "im_A": F.interpolate(batch["im_A"], size=(hs, ws), align_corners=False, mode="bilinear"),
                "im_B": F.interpolate(batch["im_B"], size=(hs, ws), align_corners=False, mode="bilinear")
            }
            finest_scale = 1
            # Run matcher
            if symmetric:
                corresps_r1 = self.forward_symmetric(batch_resized)
            else:
                corresps_r1 = self.forward(batch_resized, batched=True)
            ...
            if self.attenuate_cert:
                low_res_certainty = F.interpolate(
                    corresps_r1[16]["certainty"], size=(hs, ws), align_corners=False, mode="bilinear"
                )
                cert_clamp = 0
                factor = 0.5
            ...
            if self.upsample_preds:
                finest_corresps = corresps_r1[finest_scale]
                torch.cuda.empty_cache()
                scale_factor = math.sqrt(self.upsample_res[0] * self.upsample_res[1] / (self.w_resized * self.h_resized))
                batch["corresps"] = finest_corresps
                if symmetric:
